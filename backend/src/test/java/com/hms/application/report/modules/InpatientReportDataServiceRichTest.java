...
class InpatientReportDataServiceRichTest {
    ...
    @InjectMocks private InpatientReportDataService service;

    @Test
    void executeAllQueries() throws Exception {
        Map<String, Object> dummyRow = new HashMap<>() {{
            put("No clinical encounter found with ID: ", "1");
            put(" ORDER BY bo_curr.from_datetime DESC", "1");
            put("report_view_type", "1");
            put(" ORDER BY rc.name, b.name", "1");
            put("from_date", "1");
            put("detail", "1");
            put("1", "1");
            put(" ORDER BY ce.started_at DESC", "1");
            put("summary", "1");
            put("2025-01-01", "1");
            put(",                sn_pat.value AS ", "1");
            put("to_date", "1");
            put(",                COALESCE(b.name, '') AS ", "1");
            put("ce", "1");
            put("No Department", "1");
            put("b", "1");
        }};
