...
class RevenueReportDataServiceRichTest {
    ...
    @InjectMocks private RevenueReportDataService service;
    ...
    void executeAllQueries() throws Exception {
        Map<String, Object> dummyRow = new HashMap<>() {{
            ...
            put(" AND c.id = ?::UUID ", "1");
            put("summary", "1");
            put(" AND c.department_id = ?::UUID ", "1");
            put("2025-01-01", "1");
            put(" GROUP BY d.id, d.name ORDER BY total DESC", "1");
            put("to_date", "1");
            put("ALL", "1");
            put("b", "1");
        }};
