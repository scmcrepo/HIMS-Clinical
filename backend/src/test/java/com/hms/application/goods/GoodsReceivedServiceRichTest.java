...
class GoodsReceivedServiceRichTest {
    private GoodsReceivedService service;

    @BeforeEach void setUp() throws Exception { ReflectiveTestUtil.setupTenantContext(); service = ReflectiveTestUtil.createWithMocks(GoodsReceivedService.class); }
    ...
    @Test void testPrivateMethods() {
        for (Method m : GoodsReceivedService.class.getDeclaredMethods()) { m.setAccessible(true);
