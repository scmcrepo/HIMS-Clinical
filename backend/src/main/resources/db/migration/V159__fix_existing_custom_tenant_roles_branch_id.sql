-- V159__fix_existing_custom_tenant_roles_branch_id.sql
-- Assign branch_id to all existing roles that are not ADMIN, SUPERADMIN, or HOSPITAL_ADMIN,
-- pinning them to their tenant's default branch.

-- 1. Backfill branch_id for any tenant-wide roles (where branch_id IS NULL) that are not system roles
UPDATE roles r
SET branch_id = (
    SELECT id FROM branches b
    WHERE b.tenant_id = r.tenant_id AND b.is_default = true
    LIMIT 1
)
...
-- 2. Clone these branch-scoped roles from the default branch to all other non-default branches in the same tenant.
