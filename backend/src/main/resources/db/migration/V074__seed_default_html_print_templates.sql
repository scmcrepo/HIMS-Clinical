-- V074: seeds production-quality HTML print templates.
-- Placeholders use #{path} syntax resolved by PrintServiceImpl.resolvePlaceholders().
-- Model keys available:
--   profile.name / profile.address / profile.contactNo
--   date / dateTime / numberToString
--   data.* — populated by the matching service bean for each document type
...
.page{padding:10mm}
.hdr{text-align:center;padding-bottom:8px;margin-bottom:8px;border-bottom:2px solid #111}
.tag{font-size:8px;letter-spacing:2px;text-transform:uppercase;color:#777}
.hosp{font-size:15px;font-weight:700;margin:2px 0}.addr{font-size:8px;color:#777}
h2{font-size:14px;font-weight:700;text-align:center;letter-spacing:1px;text-transform:uppercase;margin:7px 0 2px;border:1.5px solid #111;padding:4px;display:inline-block;width:100%}
.rno{text-align:center;font-family:'IBM Plex Mono',monospace;font-size:9px;color:#555;margin-bottom:7px;display:block}
.row{display:flex;justify-content:space-between;padding:4px 0;border-bottom:1px dashed #ddd;font-size:10px}
.row label{color:#666}.row span{font-weight:600;font-family:'IBM Plex Mono',monospace}
.grand{display:flex;justify-content:space-between;padding:7px 0 4px;font-size:14px;font-weight:700;border-top:2px solid #111;margin-top:4px}
.words{font-size:9px;color:#666;font-style:italic;margin-top:5px}
.stamp{display:inline-block;border:2px solid #16a34a;border-radius:3px;padding:2px 10px;font-size:10px;font-weight:700;color:#16a34a;letter-spacing:2px;text-transform:uppercase;margin-top:8px}
.thank{text-align:center;margin-top:10px;font-size:9px;color:#999;letter-spacing:1px;text-transform:uppercase}
@media print{body{-webkit-print-color-adjust:exact;print-color-adjust:exact}}
</style></head>
<body><div class="page">
  <div class="hdr">
    ...
    <div class="addr">#{profile.address} &nbsp;|&nbsp; #{profile.contactNo}</div>
  </div>
  <h2>Payment Receipt</h2>
  <span class="rno">Receipt # #{data.receiptNumber}</span>
  <div class="row"><label>Patient Name</label><span>#{data.patient.fullName}</span></div>
  ...
  <div class="row"><label>Payment Date</label><span>#{data.paymentDate}</span></div>
  <div class="row"><label>Payment Mode</label><span>#{data.paymentMode}</span></div>
  <div class="row"><label>Bill Total</label><span>&#8377; #{data.billAmount}</span></div>
  <div class="row"><label>Previous Paid</label><span>&#8377; #{data.previousPaid}</span></div>
  <div class="grand"><span>Amount Collected</span><span>&#8377; #{data.amount}</span></div>
  <div class="row" style="border:none"><label>Balance</label><span>&#8377; #{data.balance}</span></div>
  <div class="words">#{numberToString} Only</div>
  <div style="text-align:center;margin-top:8px"><span class="stamp">&#10003; Received</span></div>
  <div class="thank">Thank you for your payment</div>
  <div style="text-align:center;margin-top:5px;font-size:8px;color:#bbb">#{date} &nbsp;&middot;&nbsp; #{profile.name}</div>
</div></body></html>$T$;
...
body{font-family:'IBM Plex Sans',sans-serif;font-size:10px;color:#111;background:#fff}
.page{padding:10mm}
.hdr{text-align:center;border-bottom:3px double #111;padding-bottom:8px;margin-bottom:8px}
.tag{font-size:8px;letter-spacing:2px;text-transform:uppercase;color:#666}
.hosp{font-size:15px;font-weight:700;margin:2px 0}.addr{font-size:8px;color:#777}
h2{font-size:14px;font-weight:700;text-align:center;letter-spacing:1px;text-transform:uppercase;margin:7px 0 5px;border:1px solid #111;padding:4px}
.rno{text-align:center;font-family:'IBM Plex Mono',monospace;font-size:9px;color:#555;margin-bottom:7px}
.row{display:flex;justify-content:space-between;padding:4px 0;border-bottom:1px dashed #ddd;font-size:10px}
.row label{color:#666}.row span{font-weight:600;font-family:'IBM Plex Mono',monospace}
.grand{display:flex;justify-content:space-between;padding:8px 0 4px;font-size:14px;font-weight:700;border-top:2px solid #111;margin-top:4px}
.words{font-size:9px;color:#555;font-style:italic;margin-top:4px}
.thank{text-align:center;margin-top:10px;font-size:9px;color:#888;letter-spacing:1px;text-transform:uppercase}
@media print{body{-webkit-print-color-adjust:exact;print-color-adjust:exact}}
</style></head>
<body><div class="page">
  <div class="hdr">
    <div class="tag">Inpatient Payment Receipt</div>
    <div class="hosp">#{profile.name}</div>
    <div class="addr">#{profile.address} | #{profile.contactNo}</div>
  </div>
  <h2>IP Receipt</h2>
  <div class="rno">RCT # #{data.receiptNumber}</div>
  <div class="row"><label>Patient</label><span>#{data.patient.fullName}</span></div>
  <div class="row"><label>Patient ID</label><span>#{data.patient.patientNumber}</span></div>
  <div class="row"><label>Bill No</label><span>#{data.billNumber}</span></div>
  <div class="row"><label>Admission Date</label><span>#{data.admissionDate}</span></div>
  <div class="row"><label>Payment Date</label><span>#{date}</span></div>
  <div class="row"><label>Payment Mode</label><span>#{data.paymentMode}</span></div>
  <div class="grand"><span>Amount Paid</span><span>&#8377; #{data.amount}</span></div>
  <div class="row" style="border:none"><label>Balance</label><span>&#8377; #{data.balance}</span></div>
  <div class="words">#{numberToString} Only</div>
  <div class="thank">Thank you for your payment</div>
</div></body></html>$T$;
...
INSERT INTO print_templates (id,name,document_type,print_mode,page_size,height,width,margin_top,margin_bottom,margin_left,margin_right,content,is_default,status) VALUES
  ...
  (gen_random_uuid(),'Default OP Receipt',        'OP_RECEIPT',           'HTML','A5',    '210mm','148mm','8mm', '8mm', '10mm','10mm',t_receipt,        true,1),
  (gen_random_uuid(),'Default IP Receipt',        'IP_RECEIPT',           'HTML','A5',    '210mm','148mm','8mm', '8mm', '10mm','10mm',t_ip_receipt,     true,1),
