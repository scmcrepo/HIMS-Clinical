...
public class SecurityConfig {
    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        http.cors(org.springframework.security.config.Customizer.withDefaults())
                ...
                .authorizeHttpRequests(auth -> auth
                        // /auth/mfa/verify is unauthenticated because it is the second
                        // half of logging in: the caller has proved their password but
                        // has no session yet. Its protection is the challenge itself —
                        // single-use, five-minute expiry, its own attempt counter, and
                        // it names the user, so a caller cannot pair someone else's
                        // challenge with their own account. (WO-029 / U-002)
                        .requestMatchers("/auth/login", "/auth/logout", "/auth/mfa/verify", "/auth/forgot-password/**", "/actuator/health", "/hospitalProfile/logo").permitAll()
