...
public interface PurchaseReceiptJpaRepository extends JpaRepository<PurchaseReceipt, UUID> {
    ...
    @Query("SELECT r FROM PurchaseReceipt r ORDER BY r.receiptDate DESC, r.createdAt DESC")
    List<PurchaseReceipt> findAllByOrderByReceiptDateDesc();
    ...
    @Query("SELECT r FROM PurchaseReceipt r WHERE " +
           "(CAST(:date AS LocalDate) IS NULL OR r.receiptDate = :date) AND " +
           "(CAST(:supplierId AS uuid) IS NULL OR r.supplierId = :supplierId) " +
           "ORDER BY r.receiptDate DESC, r.createdAt DESC")
    Page<PurchaseReceipt> searchGRNs(
