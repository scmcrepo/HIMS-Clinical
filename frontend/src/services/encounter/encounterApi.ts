...
export const encounterApi = {
  ...
  getInpatients: (query?: string, fromDate?: string, toDate?: string, consultantId?: string, activeOnly = false, status?: string, page = 0, size = 10, sortDirection?: string) => api.get<ApiResponse<PageResponse<EncounterSummary>>>(`${BASE}/inpatients`, { params: { query, fromDate, toDate, consultantId, activeOnly, status, page, size, sortDirection } }).then(r => r.data.data!),
  getOutpatients: (query?: string, fromDate?: string, toDate?: string, consultantId?: string, status?: string, activeOnly = false, page = 0, size = 10, sortDirection?: string) => api.get<ApiResponse<PageResponse<EncounterSummary>>>(`${BASE}/outpatients`, { params: { query, fromDate, toDate, consultantId, status, activeOnly, page, size, sortDirection } }).then(r => r.data.data!),
