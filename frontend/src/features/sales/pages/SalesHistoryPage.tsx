            ...
            <table className="w-full text-sm text-left">
              <thead>
                <tr className="bg-gray-50 border-b border-gray-100 text-gray-500 text-xs uppercase tracking-wider">
                  <th className="px-5 py-3.5 font-semibold">S.No</th>
                  <th className="px-5 py-3.5 font-semibold">Sales No</th>
                  <th className="px-5 py-3.5 font-semibold">Department</th>
                  <th className="px-5 py-3.5 font-semibold">Date</th>
                  <th className="px-5 py-3.5 font-semibold">Customer Name</th>
                  <th className="px-5 py-3.5 font-semibold">Customer Type</th>
                  <th className="px-5 py-3.5 font-semibold">Status</th>
                  {/* <th className="px-5 py-3.5 font-semibold">Sale By</th> */}
                  <th className="px-5 py-3.5 font-semibold text-right">Total Amt</th>
                ...
                {filteredSales?.map((s, idx) => {
                  ...
                  return (
                    <tr key={s.id} className="hover:bg-gray-50/50 transition-colors">
                      ...
                      <td className="px-5 py-4 font-medium text-gray-900">{s.sequenceNumber}</td>
                      <td className="px-5 py-4 text-gray-600">{deptName}</td>
                      <td className="px-5 py-4 text-gray-600">{formatDate(s.saleDate)}</td>
                      ...
                      <td className="px-5 py-4 text-gray-600 uppercase font-medium">{s.customerType || 'Walk-in'}</td>
                      <td className="px-5 py-4">
                        <span className={cn('inline-flex items-center px-2.5 py-0.5 rounded text-xs font-semibold',
                          ...
                          {s.status === 'SETTLED' ? `Settled on ${formatDate(s.saleDate)}` : 
                           s.status === 'WITH_DUE' ? `WithDue Amount of ${Number(s.dueAmount).toFixed(2)}` :
