...
import { bedTypeApi, chargeApi, categoryMasterApi } from '../../../../services/masters/masterApi';

export default function BedTypeTab() {
  ...
  const { data: pageData, isLoading } = useQuery({
    ...
    queryFn: () => bedTypeApi.getPaginated({ start: page * 10, limit: 10, ...(search ? { value: search } : {}) })
  })
  
  const { data: allCharges = [] } = useQuery({
    queryKey: ['allCharges'],
    queryFn: () => chargeApi.getAll()
  })

  const { data: chargeCategories = [] } = useQuery({
    queryKey: ['chargeCategories'],
    queryFn: () => categoryMasterApi.getAll('CHARGE')
  })

  const roomChargeCategoryIds = chargeCategories
    .filter((c: any) => c.chargeCategoryType === 'ROOM_CHARGE')
    .map((c: any) => c.id);

  const roomCharges = allCharges.filter((c: any) => roomChargeCategoryIds.includes(c.categoryId));

  const types = pageData?.content ?? []
  const totalPages = pageData?.totalPages ?? 0

  const blank = { name: '', serviceCatalogItemId: '', status: 'ACTIVE' as 'ACTIVE' | 'INACTIVE' }
  const [form, setForm] = useState(blank)

  const mut = useMutation({
    mutationFn: () => editing ? bedTypeApi.update(editing.id, form) : bedTypeApi.create(form),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['bedTypes'] }); reset(); toast({ title: editing ? 'Bed type updated successfully' : 'Bed type saved successfully', variant: 'success' }) },
    onError: (e: Error) => toast({ title: 'Error', description: e.message, variant: 'destructive' }),
  })

  function reset() { setShowForm(false); setEditing(null); setForm(blank) }
  function startEdit(t: any) { setEditing(t); setForm({ name: t.name, serviceCatalogItemId: t.serviceCatalogItemId ?? '', status: t.status === 1 ? 'ACTIVE' : t.status === 0 ? 'INACTIVE' : t.status || 'ACTIVE' }); setShowForm(true) }

  return (
    <Section
      title="Bed Types"
      description="Room categories with billing cycle configuration"
      action={
        <div className="flex gap-4 items-center">
          ...
          />
          <AddButton label="Add BedType" onClick={() => { reset(); setShowForm(true) }} />
        </div>
      }
    >
      {showForm && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 animate-in fade-in duration-150 p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden border border-gray-100 flex flex-col max-h-[90vh] animate-in zoom-in-95 duration-150">
            <div className="bg-gradient-to-r from-neutral-600 to-neutral-600 px-6 py-4 flex justify-between items-center text-white">
              <h3 className="text-lg font-bold tracking-tight">{editing ? 'Update Bed Type' : 'Create Bed Type'}</h3>
            ...
            <div className="p-6 overflow-y-auto space-y-6 flex-1 bg-gray-50/50">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4 bg-white p-5 rounded-xl border border-gray-150 shadow-sm">
                ...
                <Field label="Charge">
                  <select className={inputCls} value={form.serviceCatalogItemId} onChange={e => setForm(f => ({ ...f, serviceCatalogItemId: e.target.value }))}>
                    <option value="">Select Charge…</option>
                    {roomCharges.filter((c: any) => c.status !== 'INACTIVE' && c.status !== 0).map((c: any) => <option key={c.id} value={c.id}>{c.name}</option>)}
                  </select>
                </Field>
                {editing && (
            ...
            <div className="px-6 py-4 border-t bg-white flex justify-end gap-3">
              ...
              <button onClick={() => mut.mutate()} disabled={!form.name || mut.isPending}
                ...
                {mut.isPending ? (editing ? 'Updating…' : 'Creating…') : (editing ? 'Update Bed Type' : 'Create Bed Type')}
              </button>
            </div>
          </div>
        </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden flex flex-col mt-4">
        <Table headers={['S.NO', 'NAME', 'CHARGE', 'STATUS', 'ACTION']} className="border-0 shadow-none rounded-none border-b border-gray-200">
          {isLoading ? <LoadingRow /> : types.length === 0 ? <EmptyState label="bed types" /> :
            types.map((t: any, i: number) => {
              const chargeName = allCharges.find((c: any) => c.id === t.serviceCatalogItemId)?.name ?? '—';
              return (
                <tr key={t.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 text-gray-600 uppercase text-xs font-semibold">{chargeName}</td>
