...
export default function BedTab() {
  ...
  const { data: pageData, isLoading } = useQuery({
    queryKey: ['beds', page, search],
    queryFn: () => bedApi.getPaginated({ start: page * 10, limit: 10, ...(search ? { value: search } : {}) })
  })
  const { data: bedTypes = [] } = useQuery({ queryKey: ['bedTypes'], queryFn: bedApi.getBedTypes })
  
  const beds = pageData?.content ?? []
  const totalPages = pageData?.totalPages ?? 0

  const blank = { name: '', roomCategoryId: '', status: 'ACTIVE' }
  const [form, setForm] = useState(blank)

  const mut = useMutation({
    mutationFn: () => editing ? bedApi.update(editing.id, form.name, form.roomCategoryId, form.status as any) : bedApi.create(form.name, form.roomCategoryId),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['beds'] }); reset(); toast({ title: editing ? 'Bed updated successfully' : 'Bed saved successfully', variant: 'success' }) },
    onError: (e: Error) => toast({ title: 'Error', description: e.message, variant: 'destructive' }),
  })

  function reset() { setShowForm(false); setEditing(null); setForm(blank) }
  function startEdit(b: any) { setEditing(b); setForm({ name: b.name, roomCategoryId: b.roomCategoryId ?? '', status: b.status || 'ACTIVE' }); setShowForm(true) }
    ...
    <Section
      ...
      action={
        <div className="flex gap-4 items-center">
          ...
          />
          <AddButton label="Add Bed" onClick={() => { reset(); setShowForm(true) }} />
        </div>
      }
    >
      {showForm && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 animate-in fade-in duration-150 p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden border border-gray-100 flex flex-col max-h-[90vh] animate-in zoom-in-95 duration-150">
            <div className="bg-gradient-to-r from-neutral-600 to-neutral-600 px-6 py-4 flex justify-between items-center text-white">
              <h3 className="text-lg font-bold tracking-tight">{editing ? 'Update Bed' : 'Create Bed'}</h3>
            ...
            <div className="p-6 overflow-y-auto space-y-6 flex-1 bg-gray-50/50">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4 bg-white p-5 rounded-xl border border-gray-150 shadow-sm">
                <Field label="Bed No"><input className={inputCls} value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} /></Field>
                <Field label="Bed Type">
                  <select className={inputCls} value={form.roomCategoryId} onChange={e => setForm(f => ({ ...f, roomCategoryId: e.target.value }))}>
                    <option value="">Select type…</option>
                    {bedTypes.filter((bt: any) => bt.status !== 'INACTIVE' && bt.status !== 0).map((bt: any) => <option key={bt.id} value={bt.id}>{bt.name}</option>)}
                  </select>
                </Field>
                {editing && (
            ...
            <div className="px-6 py-4 border-t bg-white flex justify-end gap-3">
              ...
              <button onClick={() => mut.mutate()} disabled={!form.name || !form.roomCategoryId || mut.isPending}
                ...
                {mut.isPending ? (editing ? 'Updating…' : 'Creating…') : (editing ? 'Update Bed' : 'Create Bed')}
              </button>
            </div>
          </div>
        </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden flex flex-col mt-4">
        <Table headers={['S.NO', 'NAME', 'BED TYPE', 'STATUS', 'ACTION']} className="border-0 shadow-none rounded-none border-b border-gray-200">
          {isLoading ? <LoadingRow /> : beds.length === 0 ? <EmptyState label="beds" /> :
            beds.map((b: any, i: number) => (
              <tr key={b.id} className="hover:bg-gray-50">
                <td className="px-4 py-3 font-medium text-gray-500">{page * 10 + i + 1}</td>
                <td className="px-4 py-3 font-medium text-gray-900">{b.name}</td>
                <td className="px-4 py-3 text-gray-600 uppercase text-xs font-semibold">{b.roomCategoryName ?? '—'}</td>
                <td className="px-4 py-3"><StatusBadge active={b.status === 'ACTIVE' || b.status === 1} /></td>
                <td className="px-4 py-3 text-center"><EditBtn onClick={() => startEdit(b)} /></td>
              </tr>
            ))}
        </Table>
