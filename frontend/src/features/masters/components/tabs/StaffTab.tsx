...
export default function StaffTab() {
  ...
  const { data: pageData, isLoading } = useQuery({
    ...
    queryFn: () => staffApi.getPaginated({ start: page * 10, limit: 10, ...(search ? { value: search } : {}) })
  })
  ...
  const mut = useMutation({
    mutationFn: () => editing ? staffApi.update(editing.id, form) : staffApi.create(form),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['staff'] }); reset(); toast({ title: editing ? 'Staff updated successfully' : 'Staff saved successfully', variant: 'success' }) },
  ...
  return (
    <Section
      title="Staff"
      description="Non-consultant hospital staff members"
      action={
        <div className="flex gap-4 items-center">
          ...
          />
          <AddButton label="Add Staff" onClick={() => { reset(); setShowForm(true) }} />
        </div>
      }
    >
      {showForm && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 animate-in fade-in duration-150 p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden border border-gray-100 flex flex-col max-h-[90vh] animate-in zoom-in-95 duration-150">
            <div className="bg-gradient-to-r from-blue-600 to-indigo-600 px-6 py-4 flex justify-between items-center text-white">
              <h3 className="text-lg font-bold tracking-tight">{editing ? 'Update Staff' : 'Create Staff'}</h3>
