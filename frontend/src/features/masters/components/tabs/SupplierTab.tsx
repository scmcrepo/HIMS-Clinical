...
import { supplierApi, Supplier } from '../../../../services/masters/masterApi';
...
export default function SupplierTab() {
  ...
  const { data: pageData, isLoading } = useQuery({
    ...
    queryFn: () => supplierApi.getPaginated({ start: page * 10, limit: 10, ...(search ? { value: search } : {}) })
  })
  ...
  return (
    <Section
      title="Suppliers"
      description="Vendors for pharmaceutical and medical supplies"
      action={
        <div className="flex gap-4 items-center">
          ...
          />
          <AddButton label="Add Supplier" onClick={() => { reset(); setShowForm(true) }} />
        </div>
      }
    >
      {showForm && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 animate-in fade-in duration-150 p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden border border-gray-100 flex flex-col max-h-[90vh] animate-in zoom-in-95 duration-150">
            <div className="bg-gradient-to-r from-blue-600 to-indigo-600 px-6 py-4 flex justify-between items-center text-white">
              <h3 className="text-lg font-bold tracking-tight">{editing ? 'Update Supplier' : 'Create Supplier'}</h3>
                ...
                {editing && (
                  <div className="border-t border-gray-100 pt-4 mt-2 md:col-span-2">
                    ...
                    <div className="flex gap-2 mt-1">
                      <button type="button" onClick={() => setForm(f => ({ ...f, status: 1 }))} className={cn("px-4 py-2 text-xs font-bold rounded-lg border transition-all duration-150", form.status === 1 ? "bg-blue-600 text-white border-blue-600 shadow-sm" : "bg-white text-gray-700 border-gray-200 hover:bg-gray-50")}>Active</button>
                      <button type="button" onClick={() => setForm(f => ({ ...f, status: 0 }))} className={cn("px-4 py-2 text-xs font-bold rounded-lg border transition-all duration-150", form.status === 0 ? "bg-red-600 text-white border-red-600 shadow-sm" : "bg-white text-gray-700 border-gray-200 hover:bg-gray-50")}>InActive</button>
                    </div>
                  </div>
                )}
              </div>
            ...
            <div className="px-6 py-4 border-t bg-white flex justify-end gap-3">
              ...
              <button onClick={() => mut.mutate()} disabled={!form.name || mut.isPending}
                className="px-5 py-2 bg-blue-600 text-white text-sm font-semibold rounded-lg hover:bg-blue-700 disabled:opacity-50 transition-colors uppercase">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden flex flex-col mt-4">
        <Table headers={['S.NO', 'SUPPLIER NAME', 'CONTACT NO', 'STATUS', 'ACTION']} className="border-0 shadow-none rounded-none border-b border-gray-200">
          {isLoading ? <LoadingRow /> : types.length === 0 ? <EmptyState label="suppliers" /> :
