    ...
    >
      {showForm && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 animate-in fade-in duration-150 p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-[95vw] lg:max-w-4xl overflow-hidden border border-gray-100 flex flex-col max-h-[90vh] animate-in zoom-in-95 duration-150">
            ...
            <div className="p-6 overflow-y-auto space-y-6 flex-1 bg-gray-50/50">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4 bg-white p-5 rounded-xl border border-gray-150 shadow-sm">
                ...
                <div className="flex items-center gap-2 pt-8">
                  <input type="checkbox" id="requiresBatch" checked={form.requiresBatch} onChange={e => setForm(f => ({ ...f, requiresBatch: e.target.checked }))} className="rounded border-gray-300 text-neutral-600 focus:ring-neutral-500" />
                ...
                {editing && (
                  <div className="border-t border-gray-100 pt-4 mt-2 md:col-span-2">
                    ...
                    <div className="flex gap-2 mt-1">
                      <button type="button" onClick={() => setForm(f => ({ ...f, status: 1 }))} className={cn("px-4 py-2 text-xs font-bold rounded-lg border transition-all duration-150", (form.status === 1 || form.status === 'ACTIVE') ? "bg-neutral-600 text-white border-neutral-600 shadow-sm" : "bg-white text-gray-700 border-gray-200 hover:bg-gray-50")}>Active</button>
            ...
            <div className="px-6 py-4 border-t bg-white flex justify-end gap-3">
              ...
              <button onClick={() => mut.mutate()} disabled={!form.name || mut.isPending} className="px-5 py-2 bg-neutral-600 text-white text-sm font-semibold rounded-lg hover:bg-neutral-700 disabled:opacity-50 transition-colors">
