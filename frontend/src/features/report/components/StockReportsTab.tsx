...
export function StockReportsTab({ onViewReport }: StockReportsTabProps) {
  ...
  return (
    <div className="space-y-1">
      ...
      <ReportCard title="Expiry Stock Report" reportName="expired_items" hideFilters onViewReport={onViewReport}
        renderSummary={(data) => {
      ...
      <ReportCard title="Nearing Expiry Report" reportName="items_expiring_month" hideFilters onViewReport={onViewReport}
        renderSummary={(data) => {
      ...
      <ReportCard title="Slow Moving Items" reportName="slow_moving_items" hideFilters onViewReport={onViewReport}
        renderSummary={(data) => {
          if (!data || data.length === 0) return <div className="text-sm text-gray-500 italic px-2">No slow moving items.</div>
          return <div className="text-sm font-medium text-gray-700 bg-gray-50 border p-3 rounded-lg">{data.length} items.</div>
        }} />
      <ReportCard title="Nil Stock Report" reportName="zero_stock_items" onViewReport={onViewReport}
        renderSummary={(data) => {
